...
async fn quest_required_creature_loot_is_not_generated_after_completion_like_cpp() {
    ...
    session.quest_test_fixture_like_cpp.player_quests.get_mut(&quest_id).unwrap().status =
    ...
    let status = session.quest_test_fixture_like_cpp.player_quests.get_mut(&quest_id).unwrap();
