...
fn quest_status_projection_persists_only_nonzero_storage_objectives_like_cpp() {
    ...
    let projected = session.represented_quest_status_persistence_like_cpp(
        session.quest_test_fixture_like_cpp.player_quests.get(&quest_id).unwrap(),
    );
...
async fn quest_load_keeps_the_seven_stage_order_behind_the_typed_port_like_cpp() {
    ...
    assert_eq!(session.quest_test_fixture_like_cpp.player_quests[&active_id].objective_counts, vec![3]);
    assert!(session.quest_test_fixture_like_cpp.rewarded_quests.contains(&rewarded_id));
    assert!(session.quest_test_fixture_like_cpp.daily_quests_completed_like_cpp.contains(&daily_id));
    assert!(
        session
...
fn save_to_db_quest_status_list_skips_rewarded_non_repeatable_active_duplicate_like_cpp() {
    ...
    session.quest_test_fixture_like_cpp.rewarded_quests.insert(active_rewarded_quest_id);
...
fn quest_load_removes_active_rewarded_duplicate_and_compacts_slots_like_cpp() {
    ...
    session.quest_test_fixture_like_cpp.rewarded_quests.insert(duplicate_quest_id);

    assert_eq!(
        session.remove_represented_active_rewarded_duplicates_like_cpp(),
        vec![duplicate_quest_id]
    );
    assert!(!session.quest_test_fixture_like_cpp.player_quests.contains_key(&duplicate_quest_id));
    assert_eq!(
        session
...
fn can_take_quest_blocks_daily_already_completed_like_cpp() {
    ...
    let store = QuestStore::from_quests_like_cpp([quest.clone()]);
    session.set_quest_store(Arc::new(store));
    session.quest_test_fixture_like_cpp.daily_quests_completed_like_cpp.insert(quest.id);
...
fn can_take_quest_blocks_df_quest_already_completed_like_cpp() {
    ...
    let store = QuestStore::from_quests_like_cpp([quest.clone()]);
    session.set_quest_store(Arc::new(store));
    session.quest_test_fixture_like_cpp.df_quests_like_cpp.insert(quest.id);
...
fn can_take_quest_blocks_weekly_already_completed_like_cpp() {
    ...
    let store = QuestStore::from_quests_like_cpp([quest.clone()]);
    session.set_quest_store(Arc::new(store));
    session.quest_test_fixture_like_cpp.weekly_quests_completed_like_cpp.insert(quest.id);
...
fn can_take_quest_blocks_monthly_already_completed_like_cpp() {
    ...
    let store = QuestStore::from_quests_like_cpp([quest.clone()]);
    session.set_quest_store(Arc::new(store));
    session.quest_test_fixture_like_cpp.monthly_quests_completed_like_cpp.insert(quest.id);
...
fn can_take_quest_exclusive_group_blocks_when_peer_rewarded_non_repeatable_like_cpp() {
    ...
    session.quest_test_fixture_like_cpp.rewarded_quests.insert(peer.id);
...
fn can_take_quest_dependent_previous_rewarded_allows_like_cpp() {
    ...
    session.quest_test_fixture_like_cpp.rewarded_quests.insert(prev_id);
    assert!(session.can_take_quest(&quest));
}
#[test]
