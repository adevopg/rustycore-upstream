...
fn reset_seasonal_missing_quest_v2_store_removes_without_inventing_bit_like_cpp() {
    ...
    assert_eq!(outcome.completed_bit_clear_unrepresented, 0);
    assert_eq!(session.seasonal_quest_bucket_like_cpp(7), None);
    assert!(session.quest_test_fixture_like_cpp.represented_quest_completed_bits_like_cpp.is_empty());
}
#[test]
...
fn canonical_access_requirement_uses_team_quest_reward_like_cpp() {
    ...
    session.instance_test_fixture_like_cpp.represented_raid_difficulty_id_like_cpp = 3;
    ...
    session.quest_test_fixture_like_cpp.rewarded_quests.insert(200);
    assert!(matches!(
        session.ensure_canonical_world_map_for_current_player_like_cpp(),
...
async fn quest_giver_choose_reward_missing_source_rejects_before_mutation_like_cpp() {
    ...
    assert_eq!(
        session.quest_test_fixture_like_cpp.player_quests.get(&9_224).map(|quest| quest.status),
        Some(crate::conditions::QUEST_STATUS_COMPLETE_LIKE_CPP)
    );
    assert!(!session.quest_test_fixture_like_cpp.rewarded_quests.contains(&9_224));
...
async fn quest_giver_choose_reward_auto_complete_player_source_is_not_blocked_like_cpp() {
    ...
    assert!(!session.quest_test_fixture_like_cpp.player_quests.contains_key(&9_226));
    assert!(session.quest_test_fixture_like_cpp.rewarded_quests.contains(&9_226));
