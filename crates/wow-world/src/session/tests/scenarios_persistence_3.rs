...
fn load_rest_state_clears_stale_rest_flags_between_characters_like_cpp() {
    ...
    assert_ne!(session.rest_mgr_test_fixture_like_cpp.represented_rest_time_secs_like_cpp, 0);
    ...
    assert!(!session.represented_is_resting_like_cpp());
    assert_eq!(session.rest_mgr_test_fixture_like_cpp.represented_inn_area_trigger_id_like_cpp, 0);
    assert_eq!(session.rest_mgr_test_fixture_like_cpp.represented_rest_time_secs_like_cpp, 0);
    assert!(
        !session
            .canonical_player_has_player_flag_like_cpp(guid, PLAYER_FLAGS_RESTING_LIKE_CPP)
            .unwrap_or(true)
