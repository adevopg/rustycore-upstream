...
fn load_seasonal_quest_status_clears_stale_state_and_resets_changed_on_empty_like_cpp() {
    ...
    session.quest_test_fixture_like_cpp.seasonal_quest_changed_like_cpp = true;
    ...
    assert!(session.quest_test_fixture_like_cpp.seasonal_quests_like_cpp.is_empty());
...
fn load_seasonal_quest_status_valid_row_populates_and_blocks_can_take_like_cpp() {
    ...
    assert_eq!(
        session.quest_test_fixture_like_cpp.represented_quest_completed_bits_like_cpp,
        BTreeSet::from([65])
    );
    assert_eq!(
        session
