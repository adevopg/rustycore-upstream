//! DB2 (WDC4) loading for the extractor: TrinityCore `DB2CascFileSource`
//! (`src/tools/extractor_common/DB2CascFileSource.cpp`, files opened with
//! `zerofillEncryptedParts = true`, `HandleEncryptedSection` -> `HasTactKey`) and
//! `DB2FileLoader::LoadHeaders` / `DB2FileLoader::Load`
//! (`src/common/DataStores/DB2FileLoader.cpp`) for the two tables `vmap4_extractor`
//! reads, with the `ExtractorDB2LoadInfo.h` metadata of `MapLoadInfo` and
//! `GameobjectDisplayInfoLoadInfo`.
//!
//! The loader (header validation, encrypted-section skipping, copy table,
//! `RecordGetString`) is copied from `tools/wow-map-extractor/src/db2.rs` (same port,
//! kept per-tool on purpose); numeric fields come from [`Wdc4Reader`].
//!
//! Field indices are the physical WDC4 fields, i.e. the index into
//! `DB2Meta::MetaFields` (`ID` is not stored in the record, `IndexField == -1`). They
//! match RustyCore's own loaders for the same 3.4.3 tables
//! (`crates/wow-data/src/map.rs` `MapStore::load`,
...
pub(crate) const MAP_FIELD_DIRECTORY: usize = 0;
/// `Map.db2` `MapName` (`MetaFields[1]`, `FT_STRING`).
pub(crate) const MAP_FIELD_MAP_NAME: usize = 1;
/// `Map.db2` `ParentMapID` (`MetaFields[12]`, signed `FT_SHORT`).
pub(crate) const MAP_FIELD_PARENT_MAP_ID: usize = 12;
/// `Map.db2` `CosmeticParentMapID` (`MetaFields[13]`, signed `FT_SHORT`).
pub(crate) const MAP_FIELD_COSMETIC_PARENT_MAP_ID: usize = 13;

/// `GameobjectDisplayInfoLoadInfo::MetaInstance{ 1266277, -1, 6, 6, 0xB59CF0B2, ... }`.
pub(crate) const GAMEOBJECT_DISPLAY_INFO_META: Db2Meta = Db2Meta {
...
pub(crate) const GAMEOBJECT_DISPLAY_INFO_FIELD_FILE_DATA_ID: usize = 2;

/// A failed DB2 load.
#[derive(Debug)]
pub(crate) struct Db2LoadError {
    /// `CASC::HumanReadableCASCError(GetCascError())`.
    pub(crate) casc_error: String,
    /// `exception::what()`.
    pub(crate) what: String,
}

/// `DB2CascFileSource(storage, fileDataId)` + `DB2FileLoader::Load(&source, &loadInfo)`.
///
/// The file is read with encrypted frames zero-filled (`zerofillEncryptedParts`), and
/// sections whose `TactId` key is unknown are skipped (`HandleEncryptedSection`).
pub(crate) fn load(casc: &dyn CascSource, meta: &Db2Meta) -> Result<Db2Table, Db2LoadError> {
    let not_open = |casc_error: String| {
        eprintln!(
            "Failed to open 'FileDataId {}' in CASC storage: {casc_error}",
            meta.file_data_id
        );
        Db2LoadError {
            casc_error,
    ...
    let bytes = match casc.open_db2(meta.file_data_id) {
        Ok(Some(bytes)) => bytes,
        Ok(None) => return Err(not_open("FILE_NOT_FOUND".to_owned())),
        Err(error) => return Err(not_open(error)),
    };
    Db2Table::load(bytes, meta, |key| casc.has_tact_key(key)).map_err(|what| Db2LoadError {
