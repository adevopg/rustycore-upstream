//! Bob Jenkins' lookup3 `hashlittle`/`hashlittle2` (`CascLib`
//! `dep/CascLib/src/jenkins/lookup3.c`), needed to write the local `.idx`
//! guarded blocks and the 0x1E-byte `data.###` entry headers.
//!
//! Same byte-wise port as `wow-casc`'s private `jenkins.rs` (which is not
//! exported); both are checked against lookup3's `driver5()` vectors.
